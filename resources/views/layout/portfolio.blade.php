@extends('layout.sidebar')

@section('content')
    <!-- Tambahkan link CSS AOS jika belum ada di layout.sidebar -->
    <link href="{{ asset('assets/vendor/aos/aos.css') }}" rel="stylesheet">

    <section id="portfolio-details" class="portfolio-details section">
        @if ($selectedItem)
            <div class="container" data-aos="fade-up" data-aos-delay="100">
                <div class="row gy-4">
                    <div class="col-lg-8">
                        <div class="portfolio-details-slider swiper init-swiper">

                            <script type="application/json" class="swiper-config">
                {
                  "loop": true,
                  "speed": 600,
                  "autoplay": {
                    "delay": 5000
                  },
                  "slidesPerView": "auto",
                  "pagination": {
                    "el": ".swiper-pagination",
                    "type": "bullets",
                    "clickable": true
                  }
                }
              </script>

                            <div class="swiper-wrapper align-items-center">

                                <div class="swiper-slide">
                                    <img src="{{ asset('storage/' . $selectedItem->image) }}" alt="">
                                </div>
                            </div>
                            <div class="swiper-pagination"></div>
                        </div>
                    </div>
                    <div class="col-lg-4">
                        <div class="portfolio-info" data-aos="fade-left" data-aos-delay="200">
                            <h3>Project information</h3>
                            <ul>
                                <li><strong>Category</strong>: {{ $selectedItem->category }}</li>
                                <li><strong>Client</strong>: {{ $selectedItem->client }}</li>
                                <li><strong>Project date</strong>: {{ $selectedItem->date }}</li>
                                <li><strong>Project URL</strong>: <a href="{{ $selectedItem->link }}"> {{ $selectedItem->link }}</a></li>
                            </ul>
                        </div>
                        <div class="portfolio-description" data-aos="fade-up" data-aos-delay="300">
                            <h2>{{ $selectedItem->title }}</h2>
                            <p>
                                {{ $selectedItem->description }}
                            </p>
                        </div>
                    </div>
                </div>
            </div>
        @else
            <div class="container">
                <div class="alert alert-warning mt-5" data-aos="fade-in">
                    Portfolio item tidak ditemukan.
                </div>
            </div>
        @endif
    </section>
    <!-- /Portfolio Details Section -->

    <!-- Tambahkan script JS AOS -->
    <script src="{{ asset('assets/vendor/aos/aos.js') }}"></script>
    <script>
        AOS.init();
    </script>
@endsection
